...
This script aims to automate the data generation process of the F1 2021 game. The script
interacts with the user's GUI while playing in a simple way and may not be used for any other
purposes other than for generating data offline.
...
def get_sequence(file_name):
    ...
    for x in list_event:
        ...
        str_actions.append({"action": concrete_action, "delay": x.time - prev_time})
        prev_time = x.time
    return str_actions
...
def parse_actions(actions_list):
    ...
    for x in actions_list:
        ...
        parsed_actions.append({"action_type": action_type, "key_stroke": key_stroke, "delay": x["delay"]})
...
if __name__ == "__main__":
    ...
    for _x in range(23):  # all circuits
        for _x in range(3):
            advance_practice()
